...
- `exploration/custom/INDEX.md`, `exploration/custom/PROGRESS.md` — edit freely (same posture as standard exploration topics). The skill handles bookkeeping: directory creation, appending INDEX.md hook lines, adding/flipping PROGRESS.md rows. It does **not** author content inside `exploration/custom/<sub-exploration>/*.md` files — those are user-authored. See `reference/custom-shape.md` and `procedures/custom.md`.
